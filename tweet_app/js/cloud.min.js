...
function generate() {
    layout.font('Impact').spiral(false), fontSize = d3.scale.linear().range([10, 100]), tags.length && fontSize.domain([+tags[tags.length - 1].value || 1, +tags[0].value]), complete = 0, statusText.style("display", null), words = [], layout.stop().words(tags.slice(0, max = Math.min(tags.length, +0.0))).start()
}
function progress(t) {
...
d3.select("#random-palette").on("click", function() {
    ...
    function t() {
        c = +2, u = Math.max(-90, Math.min(90)), i = Math.max(-90, Math.min(90)), e()
    }
    function e() {
